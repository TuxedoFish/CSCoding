...
public class TwoBitSplit {
	...
	private ArrayList<Integer> A_0 = new ArrayList<Integer>(); //0
	private ArrayList<Integer> A_1 = new ArrayList<Integer>(); //1

	//B array only matches with A so we need to differentiate them
	private ArrayList<Integer> B_0 = new ArrayList<Integer>(); //0
	private ArrayList<Integer> B_1 = new ArrayList<Integer>(); //1
	
	private TwoBitSplit[] children;

	private boolean childA, childB, bottom = false;
	private int depthA, depthB = 0;
	...
	public int split(int[] portfolios, ArrayList<Integer> indices_A, ArrayList<Integer> indices_B) {
		//Initialise shift
		int shift = (int) (Question1.NUMBER_OF_BITS - (Question1.BITS_OBSERVED*(depth+1)));
		//Loop through the ints corresponding to indices_A 
		...
		//Loop through the ints corresponding to indices_B
		for(int j=0; j<indices_B.size(); j++) {
			//Take first 2 elements and place into the 4 arrays to pass into child node
			int indexB = indices_B.get(j);
		...
		if(A_0.size()>0 && B_1.size()>0) { children[0] = new TwoBitSplit((int) (this.depth+1)); childA = true; } 
		if(A_1.size()>0 && B_0.size()>0) { children[1] = new TwoBitSplit((int) (this.depth+1)); childB = true; }

		//If no children we are at the end of our search
		bottom = (!childA && !childB) || (depth==16);
	...
	public ArrayList<Match> getPossibleMatches(int []portfolios, ArrayList<Integer> indexesA, ArrayList<Integer> indexesB) {
	...
	public ArrayList<Match> getNextConsecutiveRun(int[] portfolios, ArrayList<Match> solutions, 
			ArrayList<Integer> indexesA, ArrayList<Integer> indexesB) {
